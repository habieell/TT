...
const CheckoutSummary = ({ checkoutTotal }) => {
  const { checkedItemCount, clearBasket, dispatch: basketDispatch } = useContext(BasketContext);

  const handleCheckout = () => {
    alert("Checkout success!");
    clearBasket(basketDispatch);
    if (checkoutCount === itemsCount) {
      basketDispatch({ type: 'CHECKOUT_SUCCESS' });
    }
  };
